...
EID<-read.csv("Emerging_diseases_project/EID.csv", header=TRUE)
head(EID)
#View(EID)
...
dat1<-data.frame("Year"=dat$event_year, "Type"=dat$path_type, "Count"=dat$Emerging_Disease)
head(dat1)
...
library(rstanarm)
bayesFit<-stan_glm(Count~Year+Type+Year:Type, family=poisson(link="log"), data=d3)
bayesFit$coefficients
fit<-glm(Count~Year+Type+Year:Type, family=poisson(link="log"), data=d3)
...
ggplot()+
  ...
  geom_point(mapping=aes(x=Year, y=Count, colour=Type), alpha=0.5, data=d3)+
...
#length(unique(dat1$Type))
dd<-read.csv("Emerging_diseases_project/EID_Clean.csv",header=TRUE)
dd
ggplot(data = dd) + 
